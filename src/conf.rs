...
impl Configuration {
    fn load(filepath: &str) -> Self {
        ...
        let mut config_data: Configuration = serde_json::from_str(&config_data).expect("Invalid formatting in Config.json!");
    ...
    pub fn validate_config (&mut self) {
        ...
        remove.iter().rev().for_each(|i| { self.users.remove(*i); });
        ...
        for i in 0..self.users.len() {
            ...
            self.users[i].reminders.sort_by(|a, b| a.light_on.cmp(&b.light_on));
        }
